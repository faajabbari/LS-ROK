...
from iCIFAR1002 import iCIFAR10
import tqdm
import os
import glob
import random
...
class protoAugSSL:
    def __init__(self, args, file_name, feature_extractor, task_size, device):
        ...
        color_adds = self.args.color_path
        self.colors = []
        [self.colors.append(pil_loader(color_add).resize((32, 32))) for color_add in sorted(glob.glob(os.path.join(color_adds, '*')))]
        ...
        self.bg_transform = transforms.Compose([transforms.Resize((32, 32)),
                    transforms.ToTensor()])
        self.train_dataset = iCIFAR10('./dataset', self.args.tr_path, transform=self.train_transform, download=True, colors = self.colors)
    ...
    def get_random_tr(self, img1, i):
        #plt.imshow(img1); plt.savefig('bg.png')
        ...
        # plt.imshow(final_img); plt.savefig('final.png')
        return final_img
